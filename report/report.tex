...
\end{comment}
	...
	is\_expr\_op							& $\to$ <is\_unary\_op : unary> $\vee$ \\ % put ... to indicate newline? (see .pdf)
											& <is\_binary\_op : binary> $\vee$ <is\_ternary\_op : ternary> \\
	is\_for									& $\to$ ($\emptyset$ $\vee$ <is\_for\_init : init>) ($\emptyset$ $\vee$ <is\_for\_cond : cond>) \\ % put ... to indicate newline? (see .pdf)
	...
	is\_func\_def							& $\to$ <is\_type\_decl : type> <is\_id : id> \\ % put ... to indicate newline? (see .pdf)
